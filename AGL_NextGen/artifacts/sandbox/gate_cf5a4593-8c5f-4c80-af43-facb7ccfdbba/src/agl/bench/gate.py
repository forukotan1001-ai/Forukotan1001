...
def _parse_suite_list(value: str) -> List[str]:
    # Accept comma-separated lists (and tolerate whitespace/newlines).
    return _parse_regression_suites((value or "").replace("\n", ","))
...
def run_gate_once(config: GateConfig) -> Dict[str, Any]:
    ...
    # If we're going to apply to live code, require more consistent measurement.
    min_repeats_for_apply_live = int(os.getenv("AGL_SELF_IMPROVE_APPLY_LIVE_MIN_REPEATS", "3") or "3")
    effective_repeats = int(config.repeats)
    if bool(config.apply_live):
        effective_repeats = max(effective_repeats, max(1, min_repeats_for_apply_live))

    suites_raw = os.getenv("AGL_SELF_IMPROVE_GATE_SUITES", "").strip() or str(config.suite)
    suites = _parse_suite_list(suites_raw)
    if not suites:
        suites = [str(config.suite or "deterministic").strip() or "deterministic"]

    baseline_summaries: Dict[str, Any] = {}
    for s in suites:
        baseline_summaries[s] = _run_bench_summary(base_root, s, effective_repeats, config.use_core_consciousness)

    baseline = {
        "suite": config.suite,
        "suites": suites,
        "summaries": baseline_summaries,
        "avg_accuracy": _mean([float(v.get("accuracy") or 0.0) for v in baseline_summaries.values()]),
        "regressions": {},
    }
    ...
    for s in regression_suites:
        baseline["regressions"][s] = _run_bench_summary(base_root, s, effective_repeats, config.use_core_consciousness)
    ...
    for s in suites:
        after_summaries[s] = _run_bench_summary(sandbox_root, s, effective_repeats, config.use_core_consciousness)

    after = {
        "suite": config.suite,
        "suites": suites,
        "summaries": after_summaries,
        "avg_accuracy": _mean([float(v.get("accuracy") or 0.0) for v in after_summaries.values()]),
        "regressions": {},
    }
    for s in regression_suites:
        after["regressions"][s] = _run_bench_summary(sandbox_root, s, effective_repeats, config.use_core_consciousness)

    base_acc = float(baseline.get("avg_accuracy") or 0.0)
    after_acc = float(after.get("avg_accuracy") or 0.0)

    main_ok = True
    main_details: Dict[str, Any] = {}
    for s in suites:
        b = float((baseline_summaries.get(s) or {}).get("accuracy") or 0.0)
        a = float((after_summaries.get(s) or {}).get("accuracy") or 0.0)
        ok = a >= b
        main_details[s] = {"baseline": b, "after": a, "ok": ok}
        if not ok:
            main_ok = False
    ...
    accepted = (after_acc > base_acc) and main_ok and regress_ok
    ...
    if accepted and bool(config.apply_live):
        ...
        if os.getenv("AGL_SELF_IMPROVE_APPLY_LIVE", "0").strip() != "1":
            ...
            if effective_repeats < 3:
                live_apply_note = "apply_live_blocked_min_repeats"
            else:
                try:
                    ...
                    try:
                        ...
                        live_rb.restore(live_point)  # type: ignore
                    except Exception:
                        pass
