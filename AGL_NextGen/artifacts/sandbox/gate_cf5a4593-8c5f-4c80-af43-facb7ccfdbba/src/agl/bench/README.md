...
- `AGL_BENCH_SUITE` اختيار نوع القياس:
  ...
  - سويتات الشروط الخمسة (Criteria):
    - `criteria_all`: يشغّل الخمسة معًا
    - `criteria_1_autonomy|criteria_2_superiority|criteria_3_self_learning|criteria_4_subgoals|criteria_5_causal`
...
- إعدادات:
  ...
  - `AGL_SELF_IMPROVE_GATE_SUITES` (اختياري): قائمة سويتات مفصولة بفواصل لتشغيلها جميعًا في قرار واحد (مثل: `criteria_all` أو `criteria_1_autonomy,criteria_4_subgoals`)
  - `AGL_SELF_IMPROVE_REGRESSION_SUITES` (افتراضي: `planner,deterministic`)
  - `AGL_SELF_IMPROVE_GATE_NO_SYSTEM` (افتراضي: `1`) لتشغيل التقييم بسرعة بدون Boot كامل
    - اجعلها `0` لو تريد قياس النظام الكامل (أبطأ بكثير)
  - عند تفعيل التطبيق على الكود الحقيقي (Apply-Live) يتم فرض حد أدنى للتكرار:
    - `AGL_SELF_IMPROVE_APPLY_LIVE_MIN_REPEATS` (افتراضي: `3`)
- السجل:
