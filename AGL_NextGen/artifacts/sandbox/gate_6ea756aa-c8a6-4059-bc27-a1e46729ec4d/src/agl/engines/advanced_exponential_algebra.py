...
try:
    ...
    # Fallback or mock if tensor_utils is missing
    def to_numpy_safe(x): return np.array(x)
    def to_torch_complex64(x): return torch.tensor(x, dtype=torch.complex64)
    def matmul_safe(a, b): return a @ b
    def safe_matmul(a, b): return a @ b
    def safe_sub(a, b): return a - b
    def safe_zeros_like_dtype(x): return torch.zeros_like(x)
