...
from agl.engines.learning_system.Fitness_Criteria import score_result
from agl.engines.learning_system.Uncertainty_Propagation import pred_confidence_intervals
...
class SelfEngineer:
    ...
    def meta_improvement_cycle(self, test_reports: Optional[List[Dict[str, Any]]]=None, max_candidates: int=None, min_impact_score: float=0.15) -> List[Dict[str, Any]]:
        """
        ÙŠÙ‚ÙˆØ¯ Ø¯ÙˆØ±Ø© ØªØ­Ø³ÙŠÙ† Ù…ÙŠØªØ§ ØªØ¹ØªÙ…Ø¯ Ø¹Ù„Ù‰ Ù†ØªØ§Ø¦Ø¬ Ø§Ù„Ø§Ø®ØªØ¨Ø§Ø±Ø§Øª ÙˆØ§Ù„ØªØºØ·ÙŠØ© ÙˆØ³Ø¬Ù„ Ø§Ù„ØªØ­Ø³ÙŠÙ†Ø§Øª.
        """
    ...
    def _generate_improvement_candidates(self, analyzed: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for tgt in top_targets:
            if 'Advanced_Exponential_Algebra.py' in tgt or 'tensor_utils.py' in tgt or 'Core_Engines' in tgt:
                patch = '\n# Ù…Ø«Ø§Ù„ Ø±Ù‚Ø¹Ø© ØªØ­Ø³ÙŠÙ† Ø³Ù„Ø§Ù…Ø© Ø§Ù„Ø¹Ù…Ù„ÙŠØ§Øª Ø§Ù„Ù…ØµÙÙˆÙÙŠØ©\nfrom Core_Engines.tensor_utils import safe_matmul, safe_zeros_like\n\ndef lie_bracket(self, A, B):\n    """Ù‚ÙˆØ³ Ù„ÙŠ: [A, B] = AB - BA (Ù†Ø³Ø®Ø© Ø¢Ù…Ù†Ø© backend-agnostic)"""\n    AB = safe_matmul(torch, A, B)\n    BA = safe_matmul(torch, B, A)\n    return AB - BA\n\ndef structure_constants(self, basis_elements):\n    """Ø«ÙˆØ§Ø¨Øª Ø§Ù„Ø¨Ù†ÙŠØ© Ø¨Ø§Ø³ØªØ®Ø¯Ø§Ù… safe_zeros_like Ø¹Ù†Ø¯ ØºÙŠØ§Ø¨ dtype"""\n    import torch as _torch\n    n = len(basis_elements)\n    proto = basis_elements[0]\n    try:\n        dt = getattr(proto, "dtype", None) or (_torch.float32 if hasattr(_torch, "float32") else None)\n        sc = _torch.zeros((n, n, n), dtype=dt) if dt is not None else _torch.zeros((n, n, n))\n    except Exception:\n        sc = _torch.zeros((n, n, n))\n    return sc\n'.strip()
                candidates.append({'title': 'ØªØ¹Ø²ÙŠØ² Ø³Ù„Ø§Ù…Ø© Ø¹Ù…Ù„ÙŠØ§Øª Ø§Ù„Ù…ØµÙÙˆÙØ§Øª', 'component': 'Core_Engines/Advanced_Exponential_Algebra.py:lie_bracket,structure_constants', 'rationale': 'ØªÙ‚Ù„ÙŠÙ„ Ø£Ø®Ø·Ø§Ø¡ backend (TensorLike/dtype) Ø¹Ø¨Ø± Ø·Ø¨Ù‚Ø© Ø£Ù…Ø§Ù† Ù…ÙˆØ­Ù‘Ø¯Ø©.', 'change_type': 'safety', 'suggested_patch': patch, 'impact_score': min(1.0, 0.4 + weights.get(tgt, 0) / 1.5), 'metadata': {'target': tgt}})
                break
        ...
        if low_cov_files:
            test_patch = f"""\n# Ø¥Ø¶Ø§ÙØ© Ø§Ø®ØªØ¨Ø§Ø±Ø§Øª Ø§Ø³ØªÙ‚Ø±Ø§Ø¦ÙŠØ© Ù„Ø±ÙØ¹ Ø§Ù„ØªØºØ·ÙŠØ©\n# Ù…Ø«Ø§Ù„: tests/test_meta_improvement_cycle_extra.py\nimport pytest\n\ndef test_meta_cycle_smoke():\n    from agl.engines.learning_system.Self_Engineer import SelfEngineer\n    se = SelfEngineer()\n    out = se.meta_improvement_cycle(test_reports=[{{"type":"pytest","payload":{{"failures":[]}}}}], max_candidates=1)\n    assert isinstance(out, list) and len(out) >= 0\n\n# ÙŠÙÙØ¶Ù‘Ù„ Ø¥Ø¶Ø§ÙØ© Ø­Ø§Ù„Ø§Øª ØªØºØ·ÙŠ Ù…Ø³Ø§Ø±Ø§Øª Ø§Ù„Ø­Ø§ÙØ© Ù„Ù„Ù…Ù„ÙØ§Øª:\n# {', '.join(low_cov_files)}\n""".strip()
            candidates.append({'title': 'Ø§Ø®ØªØ¨Ø§Ø±Ø§Øª ØªØºØ·ÙŠØ© Ø¥Ø¶Ø§ÙÙŠØ© Ù„Ù„Ù…Ù„ÙØ§Øª Ù…Ù†Ø®ÙØ¶Ø© Ø§Ù„ØªØºØ·ÙŠØ©', 'component': ', '.join(low_cov_files), 'rationale': 'Ø±ÙØ¹ Ø§Ù„ØªØºØ·ÙŠØ© ÙˆØªÙ‚Ù„ÙŠÙ„ Ø§Ù„Ø¯ÙŠÙˆÙ† Ø§Ù„ØªÙ‚Ù†ÙŠØ© Ø­ÙˆÙ„ ÙˆØ­Ø¯Ø§Øª Ø­Ø±Ø¬Ø©.', 'change_type': 'tests', 'suggested_patch': test_patch, 'impact_score': 0.3 + min(0.3, 0.05 * len(low_cov_files)), 'metadata': {'files': low_cov_files}})
        if signals.get('failures'):
            first_fail = signals['failures'][0]
            doc_patch = f'\n"""ØªÙˆØ¶ÙŠØ­ Ø­Ø§Ù„Ø§Øª Ø§Ù„Ø­Ø§ÙØ©:\n"""\n'.strip()
            candidates.append({'title': 'ØªÙˆØ«ÙŠÙ‚ Ø­Ø§Ù„Ø§Øª Ø§Ù„Ø­Ø§ÙØ© Ø¨Ù†Ø§Ø¡Ù‹ Ø¹Ù„Ù‰ Ø¥Ø®ÙØ§Ù‚Ø§Øª Ø³Ø§Ø¨Ù‚Ø©', 'component': first_fail.get('test', 'unknown'), 'rationale': 'ØªØ­Ø³ÙŠÙ† Ù‚Ø§Ø¨Ù„ÙŠØ© Ø§Ù„ØµÙŠØ§Ù†Ø© ÙˆÙˆØ¶ÙˆØ­ Ø³Ø¨Ø¨ Ø§Ù„Ù‚Ø±Ø§Ø±Ø§Øª Ø§Ù„ØªÙ‚Ù†ÙŠØ©.', 'change_type': 'docs', 'suggested_patch': doc_patch, 'impact_score': 0.2, 'metadata': {'source': 'pytest_failures'}})
        return candidates
def quick_smoke():
    se = SelfEngineer()
