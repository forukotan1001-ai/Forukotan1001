...
try:
    try:
        from agl.engines.advanced_wave_gates import AdvancedWaveProcessor
        from agl.engines.quantum_core import HeikalQuantumCore
    except ImportError as e1:
        print(f"DEBUG: First import attempt failed: {e1}")
        from engines.advanced_wave_gates import AdvancedWaveProcessor
        from engines.quantum_core import HeikalQuantumCore
except ImportError as e2:
    print(f"⚠️ Critical modules not found within agl.engines. Error: {e2}")
    sys.exit(1)

class AGLQuantumRealityTest:
    def __init__(self):
